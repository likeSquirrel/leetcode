...
/**
 * @author fansong Created on 2020/11/23 0023
 */
public class Q452 {
	/**
	 ...
	 *   示例 1：
	 *
	 * 输入：points = [[10,16],[2,8],[1,6],[7,12]] 输出：2 解释：对于该样例，x = 6 可以射爆
	 * [2,8],[1,6] 两个气球，以及 x = 11 射爆另外两个气球 示例 2：
	 *
	 ...
	 * 提示：
	 *
	 * 0 <= points.length <= 104 points[i].length == 2 -231 <= xstart < xend <=
	 * 231 - 1
	 *
	 */
	...
	public int findMinArrowShots(int[][] points) {
		...
		int arrowL = points[0][1];
		int count = 1;
		for (int i = 1; i < points.length; i++) {
			// 如果长度不够最小横坐标，换一只箭
			if (arrowL < points[i][0]) {
				count++;
				arrowL = points[i][1];
			}
		}
